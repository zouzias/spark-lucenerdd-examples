...
MAIN_JAR=${CURRENT_DIR}/target/scala-2.10/spark-lucenerdd-examples-assembly-0.0.21.jar

# Run spark shell locally
${SPARK_HOME}/bin/spark-submit   --conf "spark.executor.memory=512m" \
				--conf "spark.driver.memory=512m" \
				--master local[2] \
				--class org.zouzias.spark.lucenerdd.examples.linkage.LinkageACMvsDBLP \
				--jars ${MAIN_JAR} \
				"${MAIN_JAR}"
