...
object LinkageGooglevsAmazon extends Logging {
  ...
  def main(args: Array[String]) {
    ...
    val amazonDF = sqlContext.read.parquet("data/linkage-products1/linkage-products-amazon.parquet")
    logInfo(s"Loaded ${amazonDF.count} ACM records")
    val googleDF = sqlContext.read.parquet("data/linkage-products1/linkage-products-google.parquet")
    logInfo(s"Loaded ${googleDF.count} DBLP records")
    val groundTruthDF = sqlContext.read.parquet("data/linkage-products1/linkage-products-amazon-vs-google.parquet")
